...
AddressFunds PurseAccessor::obtain_funds(BingClient &bing_client,
                                         uint64_t requested_funds,
                                         string address) {
  auto points_value =
      bing_client.fetch_utxo(payment_address(address), requested_funds,
...
AddressFunds PurseAccessor::look_for_funds(BingClient &bing_client,
                                           uint64_t requested_funds,
                                           vector<string> addresses) {
  AddressFunds maxFunds = AddressFunds{"", 0, 0, vector<output_point>()};
  for (auto a : addresses) {
    auto funds = obtain_funds(bing_client, requested_funds, a);
